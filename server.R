...
shinyServer(function(input, output,session) {
    ...
    if(doeoption()==1){
      ...
      tagList(
        ...
        fluidRow(style='margin: 0px;',
                 column(6,
                        ...
                        wellPanel(
                          h4("Total number of available samples"),
                          numericInput("totn",label =NULL, 
                                       value="40"))),
                 
                 column(6,
                        wellPanel(
                          h4( "Target maximum relative (%) standard uncertainty"),
                          numericInput("std",label =NULL,
                                       value="2.5")))),
        
        fluidRow(style='margin: 0px;',
                 column(6,
                        wellPanel(
                          h4("The expected approximate value of the measurand, quantity of A         "),
                          numericInput(
                            "xnew",
                            label = NULL,
                            value = 0.79)
                 ...
                 column(6,
                        wellPanel(
                          h4("The Slope of  calibration line"),
                          fluidRow(
                            column(6,
                                   numericInput(
                                     "betal",
        ...
        fluidRow(style='margin: 0px;',
                 column(6,
                        wellPanel(
                          h4("Target calibration region; x-axis range in terms of concentrations of A:I in the calibration solutions"),
                          fluidRow(
                            column(6,
                                   numericInput(
                                     "a",
                 ...
                 column(6,
                        wellPanel(
                          h4("Expected standard deviation of peak area ratios (A:I) from  a calibration solution; (uy)"),
                          fluidRow(
                            column(6,
                                   numericInput(
                                     "vyl",
        ...
        fluidRow(style='margin: 0px;',
                 column(6,
                        wellPanel(
                          h4("Expected standard uncertainty of the concentration ratios of A:I in the calibration experiment; (ux)"),
                          fluidRow(
                            column(6,
                                   numericInput(
                                     "vxl",
                 ...
                 column(6,
                        wellPanel(
                          h4("Expected relative standard uncertainty of the concentration of I in samples measured for the quantitation experiment; (ui)"),
                          fluidRow(
                            column(6,
                                   numericInput(
                                     "vil",
        ...
        fluidRow(style='margin: 0px;',
                 column(6,
                        wellPanel(
                          h4("Expected relative standard uncertainty of the concentration ratios of A:I in the calibration experiment (rux)",),
                          fluidRow(
                            column(6,
                                   numericInput(
                                     "etal",
                 ...
                 column(6,
                        wellPanel(
                          h4("Expected between-sample variability (standard deviation) of peak area ratios (A:I) from the quantitation experiment(us)"),
      ...
      tagList(
        ...
        fluidRow(style='margin: 0px;',
                 ...
                 column(6,
                        wellPanel(
                          h4( "The expected Response Factor value"),
                          numericInput("b",label =NULL,
                                       value="1.32")))),
        ...
        fluidRow(style='margin: 0px;',
                 ...
                 column(6,
                        wellPanel(
                          h4("Target calibration point in terms of concentration of A:I in the calibration solution"),
                          numericInput(
                            "wacm",
                            label = NULL,
                            value = 0.8)
                 ...
                 column(6,
                        wellPanel(
                          h4("The expected approximate value of the concentration of A:I in the sample"),
                          numericInput(
                            "xnew",
                            label = NULL,
                            value = 0.79)))),
  ...
  outbayes<- eventReactive(input$go,{
    ...
    if (choice()==1){
      ...
      if(N_tables==1){
        ...
        cu<-NULL
        for(i in 1:N){
          for(j in 1:NR){cu<-c(cu,quantile(predm[,i], 0.975))}}
        cu<-rep(unique(cu), NR)
        ...
        vyll<-max(0, lineout$mean$vyp-1.96*lineout$sd$vyp)
        ...
        betal<-max(0, lineout$mean$b-1.96*lineout$sd$b)
        ...
        vsll<-max(0,lineout$mean$vsp-1.96*lineout$sd$vsp) #DOE vsl
        ...
        vill<-max(0,lineout$mean$vip-1.96*lineout$sd$vip) #DOE vil
        viuu<-lineout$mean$vip+1.96*lineout$sd$vip #DOE viu
        vil<-(vill)^2
        viu<-(viuu)^2
      ...
      #  hide_waiter()
        ...
        vyll<-max(0,lineout$mean$vyp-1.96*lineout$sd$vyp)
        vylll<-max(0,lineout$mean$vyp-1.96*lineout$sd$vyp)
        vyuu<-lineout$mean$vyp+1.96*lineout$sd$vyp
        vyuuu<-lineout$mean$vyp+1.96*lineout$sd$vyp
     
        betal<-max(0, lineout$mean$b-1.96*lineout$sd$b)
        betall<-max(0, lineout$mean$b-1.96*lineout$sd$b)
        betau<-lineout$mean$b+1.96*lineout$sd$b
        ...
        vsll<-max(0, lineout$mean$vsp-1.96*lineout$sd$vsp) #DOE vsl
        vslll<-max(0, lineout$mean$vsp-1.96*lineout$sd$vsp) #DOE vsl
        vsuu<-lineout$mean$vsp+1.96*lineout$sd$vsp #DOE vsu
       ...
       # wacmean<-data.frame(mean=mean(lineout$mean$wac))
        ...
        expr<-reactive({
          ...
          else
          {
            ...
            {
              nr<<-nrow(read.csv(input$samplefile$datapath[i]))
              nras<-ncol(read.csv(input$samplefile$datapath[i]))-2
              n1<-nr*nras
              expr[[i]]<-matrix(c(rep(1*i,each=n1)))
        ...
        for (i in 1:nfile){
          ...
          vill[[i]]=max(0,vilmean[[i]]-1.96*vilsd[[i]])
          viuu[[i]]=vilmean[[i]]+1.96*vilsd[[i]]
          xnew[[i]]=mean(mult[[i]])
  ...
  output$report <- downloadHandler(
    ...
    content = function(file) {
      ...
      params <- list(bayes=outbayes(), condition=condition) 
  ...
  # expde<- eventReactive(input$enter,{
       ...
       if(N_tables==1){
         ...
         expr<-reactive({
           ...
           else
           {
             ...
             {
               nr<<-nrow(read.csv(input$samplefile1$datapath[i]))
               nras<-ncol(read.csv(input$samplefile1$datapath[i]))-2
               n1<-nr*nras
               expr[[i]]<-matrix(c(rep(1*i,each=n1)))
   ...
   output$downloadbayesout1 <- downloadHandler(
     ...
     content = function(file) {
       ...
       # Set up parameters to pass to Rmd document
       params <- list(bayes=outbayes1(), condition=condition )
